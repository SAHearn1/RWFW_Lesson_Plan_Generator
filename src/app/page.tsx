...
export default function HomePage() {
  ...
  const [selectedCategory, setSelectedCategory] =
    useState<RootWorkEntry['category'] | 'all'>('all');
    ...
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      if (currentUser) {
        ...
        signInAnonymously(auth).catch((err) => {
          ...
          setError(
            err?.code ? `Auth error: ${err.code}` : 'Could not sign in. Please try again later.'
          );
          setView('form');
        });
      }
  ...
  const handleGeneratePlan = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const prompt = `
      ...
      **Lesson Details:**
      ...
      - **Standards Alignment Input:** ${
        standards || 'Please align with common core or relevant state standards.'
      }
    ...
    try {
      ...
      // Read raw first, then parse (avoids "Unexpected end of JSON input")
      const raw = await response.text();
      let data: any = null;
      try {
        data = raw ? JSON.parse(raw) : null;
      } catch {
        // ignore parse error; we'll use raw for error messaging below
      }
        ...
        <header className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white p-8 rounded-t-xl">
          <div className="text-center mb-6">
            ...
            <p className="text-lg opacity-90 mt-2">
              Professional Development Platform for Healing-Centered Education
            </p>
          </div>
              ...
              {view === 'form' && user && (
                <form id="form-section" onSubmit={handleGeneratePlan}>
                  <div className="bg-green-50 border border-green-200 text-green-800 p-5 rounded-lg mb-8">
                    ...
                    <p>
                      This tool generates comprehensive lesson plans grounded in trauma-informed
                      practices, healing-centered pedagogy, and culturally responsive education.
                    </p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-4">
                    <div className="form-group">
                      ...
                      <select
                        ...
                        required
                        className="w-full p-3 border-2 border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500"
                      >
                  ...
                  <div className="form-group mb-4">
                    ...
                    <select
                      ...
                      required
                      className="w-full p-3 h-40 border-2 border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500"
                    >
                      {[
                  ...
                  <button
                    type="submit"
                    disabled={isLoading}
                    className="w-full bg-indigo-600 text-white p-4 rounded-lg font-semibold text-lg hover:bg-indigo-700 transition duration-300 disabled:bg-indigo-300"
                  >
              ...
              {view === 'results' && (
                <div id="results-section">
                  ...
                  <div className="flex flex-col sm:flex-row justify-center gap-4 mb-8">
                    <button
                      onClick={() => {
                        /* Add download functionality if desired */
                      }}
                ...
                <div className="bg-slate-50 p-6 rounded-lg border-2 border-slate-200">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setNewEntry((prev) => ({ ...prev, category: e.target.value as any }))
                      }
                      className="p-3 border-2 border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500"
                    >
                      <option value="legal">Legal</option>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <select
                      ...
                      onChange={(e) =>
                        setNewEntry((prev) => ({ ...prev, priority: e.target.value as any }))
                      }
                      className="p-3 border-2 border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500"
                    >
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setNewEntry((prev) => ({ ...prev, status: e.target.value as any }))
                      }
                      className="p-3 border-2 border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500"
                    >
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNewEntry((prev) => ({ ...prev, assignee: e.target.value }))
                      }
                      className="p-3 border-2 border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500"
                    />
                  </div>
              ...
              <div className="space-y-4">
                {filteredEntries.length === 0 ? (
                  ...
                  filteredEntries.map((entry) => (
                    <div
                      key={entry.id}
                      className="bg-white border-2 border-slate-200 rounded-lg p-6 hover:shadow-md transition-shadow"
                    >
                      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                        ...
                        <div className="flex gap-2">
                          <select
                            value={entry.status}
                            onChange={(e) =>
                              updateEntryStatus(entry.id, e.target.value as any)
                            }
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="bg-blue-50 p-6 rounded-lg border border-blue-200">
                  ...
                  <p className="text-3xl font-bold text-blue-600">
                    {rootWorkFramework.entries.length}
                  </p>
                </div>
                ...
                <div className="bg-red-50 p-6 rounded-lg border border-red-200">
                  ...
                  <p className="text-3xl font-bold text-red-600">
                    {
                      rootWorkFramework.entries.filter(
                        (e) => e.complianceFlags && e.complianceFlags.length > 0
                      ).length
                    }
                  </p>
                </div>
              ...
              <div className="bg-slate-50 p-6 rounded-lg border border-slate-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                  <div>
                    ...
                    {rootWorkFramework.metadata.version}
                  </div>
                  <div>
                    <span className="font-medium">Created by:</span>{' '}
                    {rootWorkFramework.metadata.createdBy}
                  </div>
                  <div>
                    <span className="font-medium">Last Updated:</span>{' '}
                    {formatDate(rootWorkFramework.metadata.lastUpdated)}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-indigo-50 p-6 rounded-lg border border-indigo-200">
                ...
                <p className="text-indigo-800 mb-4">
                  Your Root Work Framework is ready for integration into your professional
                  development courses. Each framework entry can be transformed into course modules
                  for summer/fall 2025 deployment.
                </p>
