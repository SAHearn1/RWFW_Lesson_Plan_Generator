...
export default function GeneratorClient({ userName }: GeneratorClientProps) {
  ...
  const handleDownload = async (format: 'pdf' | 'docx') => {
    ...
    try {
      const response = await fetch(`/api/export/${format}`, {
        ...
        body: JSON.stringify({
          markdown: latestAssistantMessage,
          title: unitTitle,
        }),
      });
  ...
  const renderMarkdown = (markdown: string): string => {
    ...
    return markdown
      ...
      .replace(
        /^# (.*$)/gm,
        '<h1 class="text-3xl font-bold my-4 font-serif text-brand-deep-canopy">$1</h1>',
      )
      .replace(
        /^## (.*$)/gm,
        '<h2 class="text-2xl font-bold mt-8 mb-3 font-serif text-brand-evergreen">$1</h2>',
      )
      .replace(
        /^### (.*$)/gm,
        '<h3 class="text-xl font-bold mt-6 mb-2 text-brand-leaf">$1</h3>',
      )
            ...
            <div className='grid gap-6 sm:grid-cols-2'>
              <div>
                ...
                >
                  ...
                  <option value='Social Studies / History'>
              ...
              <div>
                ...
                <p className='mt-1 text-xs text-slate-500'>
                  Note: 1-3 days is recommended to ensure maximum detail and
                  quality.
                </p>
              </div>
            </div>
              ...
              {latestAssistantMessage && (
                <>
                  <div className='mb-6 flex flex-wrap items-center justify-between gap-4 rounded-lg bg-slate-50 p-4'>
                    <h2 className='font-serif text-xl font-bold text-brand-evergreen'>
                      Your Lesson Plan
                    </h2>
                  ...
                  <article
                    ...
                    dangerouslySetInnerHTML={{
                      __html: renderMarkdown(latestAssistantMessage),
                    }}
                  />
                </>
              )}
