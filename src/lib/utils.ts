...
export const validateRootWorkEntry = (entry: Partial<RootWorkEntry>): string[] => {
  const errors: string[] = []
...
export const checkComplianceFlags = (entry: RootWorkEntry): string[] => {
  ...
  if (entry.category === 'educational' && entry.tags.includes('special-education')) {
  ...
  if (entry.tags.includes('student-data') || entry.tags.includes('educational-records')) {
  ...
  if (entry.tags.includes('health-records') || entry.tags.includes('medical-information')) {
...
export const getPriorityScore = (priority: RootWorkEntry['priority']): number => {
  const scores = {
    'critical': 4,
    'high': 3,
    'medium': 2,
...
export const getCategoryColor = (category: RootWorkEntry['category']): string => {
  const colors = {
...
export const sortEntriesByPriority = (entries: RootWorkEntry[]): RootWorkEntry[] => {
  return [...entries].sort((a, b) => getPriorityScore(b.priority) - getPriorityScore(a.priority))
}

export const sortEntriesByDueDate = (entries: RootWorkEntry[]): RootWorkEntry[] => {
...
// Search utility
export const searchEntries = (entries: RootWorkEntry[], query: string): RootWorkEntry[] => {
...
export const exportToCSV = (entries: RootWorkEntry[]): string => {
  const headers = ['Title', 'Description', 'Category', 'Priority', 'Status', 'Due Date', 'Assignee', 'Tags']
  const csvContent = [
    headers.join(','),
    ...entries.map(entry => [
      `"${entry.title}"`,
