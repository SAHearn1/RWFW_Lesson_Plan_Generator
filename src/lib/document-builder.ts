...
export const createPdf = async (markdown: string, title: string) => {
  ...
  const drawHeader = (currentPage: any, pageNum: number) => {
    currentPage.drawRectangle({
      ...
      width,
      height: 35,
      color: brandColors.evergreen.pdf,
    });
    currentPage.drawText(`${title} | Page ${pageNum}`, {
      x: margin,
      y: height - 25,
      font: boldFont,
      size: 12,
      color: brandColors.white.pdf,
    });
  };
  ...
  lines.forEach((line) => {
    if (line.startsWith('# ')) {
      ...
      if (line.includes('[Teacher Note:')) {
        ...
        children.push(
          new TextRun({ text: line, color: brandColors.charcoal.hex }),
        );
      }
      ...
      paragraphStyles: [
        {
          ...
          run: {
            size: 32,
            bold: true,
            color: brandColors.evergreen.hex,
            font: 'Merriweather',
          },
        },
        {
        ...
        },
        {
          ...
          run: {
            size: 24,
            bold: true,
            color: brandColors.charcoal.hex,
            font: 'Inter',
          },
        },
      ],
